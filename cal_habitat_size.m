...
%      wall_thickness = .05;
%      vol_food = 72.9; 
%      recycle_equip_vol = 18.31;
%      vol_farm_water = 0;
%      vol_personal_water = 358.86;
count = 1;
...
%Calculate the volume of storage space required for water, water
%reclamation units, and food
...
if A_farm == 0
   A_farm_dome = 0;
    ...
    A_farm_dome = A_farm/(ceil(A_farm/max_area));
end
...
opt_max_half_rad = max_radius(k)*2/3;
...
opt_half_area = pi*opt_max_half_rad^2;
...
opt_num_farm_domes = ceil((A_farm/(opt_area+opt_half_area)));
...
%dome)
opt_num_domes = opt_num_farm_domes + opt_num_lab_domes + opt_num_storage_domes;
...
if opt_num_farm_domes == 0
    ...
    opt_a_farm = A_farm/ceil(A_farm/opt_area);
end
